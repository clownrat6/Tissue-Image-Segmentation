...
# TODO: Add related comments & doc string.
...
class RAdam_4step(Optimizer):
    ...
    def __init__(self,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay)
        # whether update the first 4 steps
        self.update_all = update_all
        # whether use additional 4 steps for SGD
        self.additional_four = additional_four
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if grad.is_sparse:
                    raise RuntimeError(
                        'RAdam_4step does not support sparse gradients')
                ...
                if state['step'] > 0:
                    # since this exp requires exactly 4 step, it is hard coded
                    state_step = state[
                        'step'] + 4 if self.additional_four else state['step']
                    ...
                    if state_step == buffered[0]:
                        ...
                        beta2_t = beta2**state['step']
                        # since this exp requires exactly 4 step, it is hard
                        # coded
                        if state['step'] > 4:
                            N_sma_max = 2 / (1 - beta2) - 1
                            ...
                            step_size = group['lr'] * math.sqrt(
                                (N_sma - 4) / (N_sma_max - 4) *
                                (N_sma - 2) / N_sma * N_sma_max /
                                (N_sma_max - 2)) / (1 - beta1**state_step)
                    ...
                    if state['step'] > 4:
                        ...
                        denom = (exp_avg_sq.sqrt() /
                                 math.sqrt(1 - beta2**state_step)).add_(
                                     group['eps'])
                        p_data_fp32.addcdiv_(-step_size, exp_avg, denom)
                        p.data.copy_(p_data_fp32)
                    elif self.update_all:
