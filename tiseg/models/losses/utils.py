...
def overlapping_caculation(pred,
    ...
    Args:
        ...
        metric (str): The token to select overlapping metric.
        thresh (float, optional): If not None, predictions with scores under
            this threshold are considered incorrect. Default to None.

    Returns:
        float | tuple[float]: Return iou or dice metric.
    """
    assert pred.ndim == target.ndim + 1
    assert pred.size(0) == target.size(0)
    assert metric.lower() in ['iou', 'dice']
    ...
    if metric.lower() == 'iou':
        ...
        res = 2 * 100.0 * area_intersect / (area_pred + area_label)
