...
from .fast_unet_head import UNetLayer, conv1x1, conv3x3
...
class RU(nn.Module):
    ...
    def __init__(self,
        ...
        self.residual_ops = nn.Sequential(
            conv3x3(in_dims, out_dims, norm_cfg), self.act_layer,
            conv3x3(out_dims, out_dims, norm_cfg))
...
class AU(nn.Module):
    ...
    def __init__(self, in_dims, num_masks=1):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_dims, num_masks, kernel_size=1, bias=False),
            nn.Sigmoid())
...
class DGM(nn.Module):
    ...
    def __init__(self,
        ...
        self.mask_pre_branch = RU(self.in_dims, self.feed_dims, norm_cfg,
                                  act_cfg)
        self.direction_pre_branch = RU(self.feed_dims, self.feed_dims,
                                       norm_cfg, act_cfg)
        self.point_pre_branch = RU(self.feed_dims, self.feed_dims, norm_cfg,
                                   act_cfg)
        ...
        self.direction_pred_op = nn.Conv2d(
            self.feed_dims, self.num_angles + 1, kernel_size=1)
        self.mask_pred_op = nn.Conv2d(
            self.feed_dims, self.num_classes, kernel_size=1)
