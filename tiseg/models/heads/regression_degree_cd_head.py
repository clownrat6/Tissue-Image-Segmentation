...
class DGM(nn.Module):
    ...
    def __init__(self,
        ...
        self.dir_to_mask_attn = AU(1)

        # Prediction Operations
        self.point_conv = nn.Conv2d(self.feed_dims, 1, kernel_size=1)
        self.dir_degree_conv = nn.Conv2d(self.feed_dims, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        dir_degree_logit = self.dir_degree_conv(dir_feature_with_point_logit)
        # mask branch
        mask_feature_with_dir_logit = self.dir_to_mask_attn(mask_feature, dir_degree_logit)
        mask_logit = self.mask_conv(mask_feature_with_dir_logit)

        return mask_logit, dir_degree_logit, point_logit


class RegDegreeCDHead(UNetHead):
