...
class DGM(nn.Module):
    ...
    def __init__(self,
        ...
        self.tc_mask_conv = nn.Conv2d(9, 3, kernel_size=1)
        ...
        self.mask_V = nn.Conv2d(self.num_classes, 9, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # mask branch
        tc_mask_logit = self.tc_mask_conv(mask_feature_with_att)

        return tc_mask_logit, mask_logit, dir_logit, point_logit


class MultiTaskCDVoronoiHead(UNetHead):
...
#         trunc_normal_(self.relative_position_bias_table, std=.02)
    ...
    def forward(self, xq, xk, xv, mask=None, Voronoi=None):
        ...
        k = self.k(xk).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k = k[0]
        
        v = self.v(xv).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        v = v[0]
        x = v.detach()
