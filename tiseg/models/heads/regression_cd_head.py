...
class DGM(nn.Module):
    ...
    def __init__(self,
        ...
        self.dir_to_mask_attn = AU(2)

        # Prediction Operations
        self.point_conv = nn.Conv2d(self.feed_dims, 1, kernel_size=1)
        self.dir_sin_conv = nn.Conv2d(self.feed_dims, 1, kernel_size=1)
        self.dir_cos_conv = nn.Conv2d(self.feed_dims, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        dir_sin_logit = self.dir_sin_conv(dir_feature_with_point_logit)
        dir_cos_logit = self.dir_cos_conv(dir_feature_with_point_logit)
        # mask branch
        mask_feature_with_dir_logit = self.dir_to_mask_attn(mask_feature, torch.cat((dir_sin_logit, dir_cos_logit), dim = 1))
        mask_logit = self.mask_conv(mask_feature_with_dir_logit)

        return mask_logit, dir_sin_logit, dir_cos_logit, point_logit


class RegCDHead(UNetHead):
