...
class DGM(nn.Module):
    ...
    def __init__(self,
        ...
        self.mask_all_feats = RU(self.in_dims, self.feed_dims, norm_cfg, act_cfg)
        self.dir_all_feats = RU(self.in_dims, self.feed_dims, norm_cfg, act_cfg)
        self.tc_mask_feats = RU(self.feed_dims, self.feed_dims, norm_cfg, act_cfg)
        self.mask_feats = RU(self.feed_dims, self.feed_dims, norm_cfg, act_cfg)
    ...
    def forward(self, x):
        ...
        mask_all_feature = self.mask_all_feats(x)
        dir_all_feature = self.dir_all_feats(x)

        mask_feature = self.mask_feats(mask_all_feature)
        tc_mask_feature = self.tc_mask_feats(mask_all_feature)

        dir_feature = self.dir_feats(dir_all_feature)
        point_feature = self.point_feats(dir_all_feature)
...
class MultiTaskCDHeadTwobranch(UNetHead):
