    ...
    def __init__(self, ch_in, ch_out=None, skip_in=0, use_bn=True, parametric=False):
        ...
        if parametric:
            ...
            self.conv1 = nn.Conv2d(
                in_channels=ch_in, out_channels=ch_out, kernel_size=(3, 3), stride=1, padding=1, bias=(not use_bn))
            self.bn1 = nn.BatchNorm2d(ch_out) if use_bn else None
        ...
        self.conv2 = nn.Conv2d(
            in_channels=conv2_in, out_channels=ch_out, kernel_size=(3, 3), stride=1, padding=1, bias=(not use_bn))
        self.bn2 = nn.BatchNorm2d(ch_out) if use_bn else None

    def forward(self, x, skip):

        x = self.up(x) if self.parametric else F.interpolate(
...
class Unet(nn.Module):
    ...
    def __init__(self,
        ...
        self.backbone, self.shortcut_features, self.bb_out_name = get_backbone(backbone_name, pretrained=True)
        ...
        decoder_filters = decoder_filters[:len(
            self.shortcut_features)]  # avoiding having more blocks than skip connections
    ...
    def forward(self, *input):
        ...
        x_F2_dir = self.directionAtt(x_F2, x_point)
        x_dir = self.direction_conv(x_F2_dir)

        x_F1_mask = self.maskAtt(x_F1, x_dir)
        x_final_mask = self.mask_conv(x_F1_mask)

        return x_final_mask, x_dir, x_point
