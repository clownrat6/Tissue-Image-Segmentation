...
class RegCDNetSegmentor(BaseSegmentor):
    ...
    def __init__(self, num_classes, train_cfg, test_cfg):
        super(RegCDNetSegmentor, self).__init__()
    ...
    def calculate(self, img, rescale=False):
        ...
        mask_logit, dir_sin_logit, dir_cos_logit, point_logit = self.head(bottom_feat, skip_feats)

        if rescale:
            mask_logit = resize(input=mask_logit, size=img.shape[2:], mode='bilinear', align_corners=False)
            dir_sin_logit = resize(input=dir_sin_logit, size=img.shape[2:], mode='bilinear', align_corners=False)
            dir_cos_logit = resize(input=dir_cos_logit, size=img.shape[2:], mode='bilinear', align_corners=False)
            point_logit = resize(input=point_logit, size=img.shape[2:], mode='bilinear', align_corners=False)

        return mask_logit, dir_sin_logit, dir_cos_logit, point_logit

    def forward(self, data, label=None, metas=None, **kwargs):
        """detectron2 style forward functions. Segmentor can be see as meta_arch of detectron2.
        """
        if self.training:
            mask_logit, dir_sin_logit, dir_cos_logit, point_logit = self.calculate(data['img'])
            ...
            dir_sin_gt = dir_sin_gt.squeeze(1)
            dir_cos_gt = dir_cos_gt.squeeze(1)
            ...
            dir_loss = self._reg_dir_loss(dir_sin_logit, dir_cos_logit, dir_sin_gt, dir_cos_gt, weight_map)
        ...
        for rotate_degree in self.rotate_degrees:
            for flip_direction in self.flip_directions:
                ...
                if self.test_cfg.mode == 'split':
                    sem_logit, dir_sin_logit, dir_cos_logit, point_logit = self.split_inference(img, meta, rescale)
                else:
                    sem_logit, dir_sin_logit, dir_cos_logit, point_logit = self.whole_inference(img, meta, rescale)

                sem_logit = self.reverse_tta_transform(sem_logit, rotate_degree, flip_direction)
                dir_sin_logit = self.reverse_tta_transform(dir_sin_logit, rotate_degree, flip_direction)
                dir_cos_logit = self.reverse_tta_transform(dir_cos_logit, rotate_degree, flip_direction)
                point_logit = self.reverse_tta_transform(point_logit, rotate_degree, flip_direction)

                sem_logit = F.softmax(sem_logit, dim=1)
                dir_sin_logit = dir_sin_logit.sigmoid()
                dir_cos_logit = dir_cos_logit.sigmoid()

                sem_logit_list.append(sem_logit)
                dir_sin_logit_list.append(dir_sin_logit)
                dir_cos_logit_list.append(dir_cos_logit)
    ...
    def split_inference(self, img, meta, rescale):
        ...
        dir_sin_logit = torch.zeros((B, 1, H1, W1), dtype=img.dtype, device=img.device)
        dir_cos_logit = torch.zeros((B, 1, H1, W1), dtype=img.dtype, device=img.device)
        ...
        for i in range(0, H1 - overlap_size, window_size - overlap_size):
            ...
            for j in range(0, W1 - overlap_size, window_size - overlap_size):
                ...
                sem_patch, dir_sin_patch, dir_cos_patch, point_patch = self.calculate(img_patch)
        ...
        dir_sin_logit = dir_sin_logit[:, :, (H1 - H) // 2:(H1 - H) // 2 + H, (W1 - W) // 2:(W1 - W) // 2 + W]
        dir_cos_logit = dir_cos_logit[:, :, (H1 - H) // 2:(H1 - H) // 2 + H, (W1 - W) // 2:(W1 - W) // 2 + W]
        ...
        if rescale:
            ...
            dir_sin_logit = resize(dir_sin_logit, size=meta['ori_hw'], mode='bilinear', align_corners=False)
            dir_cos_logit = resize(dir_cos_logit, size=meta['ori_hw'], mode='bilinear', align_corners=False)
            point_logit = resize(point_logit, size=meta['ori_hw'], mode='bilinear', align_corners=False)
        return sem_logit, dir_sin_logit, dir_cos_logit, point_logit
    ...
    def _reg_dir_loss(self, dir_sin_logit, dir_cos_logit, dir_sin_gt, dir_cos_gt, weight_map=None):
        dir_loss = {}
        dir_sin_logit = dir_sin_logit.sigmoid()
        dir_cos_logit = dir_cos_logit.sigmoid()
        dir_mse_loss_calculator = nn.MSELoss()
        dir_sin_mse_loss = dir_mse_loss_calculator(dir_sin_logit, dir_sin_gt)
        dir_cos_mse_loss = dir_mse_loss_calculator(dir_cos_logit, dir_cos_gt)
