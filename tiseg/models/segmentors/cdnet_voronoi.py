...
class CDNetVoronoiSegmentor(BaseSegmentor):
    """Base class for segmentors."""

    def __init__(self, num_classes, train_cfg, test_cfg):
        super(CDNetVoronoiSegmentor, self).__init__()
    ...
    def forward(self, data, label=None, metas=None, **kwargs):
        ...
        """
        if self.training:
            mask_logit, dir_logit, point_logit, foreground_logit = self.calculate(data['img'])
    ...
    def _foreground_loss(self, foreground_logit, mask_gt, weight_map=None):
        foreground_loss = {}
        foreground_ce_loss_calculator = nn.CrossEntropyLoss(reduction='none')
        foreground_dice_loss_calculator = MultiClassDiceLoss(num_classes=self.num_classes - 1)
        # Assign weight map for each pixel position
        foreground_ce_loss = foreground_ce_loss_calculator(foreground_logit, mask_gt)
        # if weight_map is not None:
        #     foreground_ce_loss *= weight_map[:, 0]
        foreground_ce_loss = torch.mean(foreground_ce_loss)
        foreground_dice_loss = foreground_dice_loss_calculator(foreground_logit, mask_gt)
        # loss weight
        alpha = 1
        beta = 0
